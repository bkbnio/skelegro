...
object ActionEntityExtensions {
  ...
  fun GithubAction.nestedEntities(
    init: ActionEntity.NestedEntitiesWrapper.() -> Unit
  ): ActionEntity.NestedEntitiesWrapper {
    val entity = ActionEntity.NestedEntitiesWrapper().apply(init)
    entities.add(entity)
    return entity
  }

  fun ActionEntity.NestedEntityWrapper.nestedEntity(
    init: ActionEntity.NestedEntity.() -> Unit
  ): ActionEntity.NestedEntity {
  ...
  fun ActionEntity.NestedEntity.nestedEntities(
    init: ActionEntity.NestedEntities.() -> Unit
  ...
  fun ActionEntity.NestedEntitiesWrapper.stringEntity(
    init: ActionEntity.StringEntity.() -> Unit
  ): ActionEntity.StringEntity {
    val entity = ActionEntity.StringEntity().apply(init)
    values.add(entity)
    return entity
  }

  fun <T> ActionEntity.NestedEntities.arrayEntity(
    init: ActionEntity.ArrayEntity<T>.() -> Unit
  ): ActionEntity.ArrayEntity<T> {
    val entity = ActionEntity.ArrayEntity<T>().apply(init)
    values.add(entity)
    return entity
  }

  fun <T> ActionEntity.NestedEntity.arrayEntity(
    init: ActionEntity.ArrayEntity<T>.() -> Unit
  ): ActionEntity.ArrayEntity<T> {
    val entity = ActionEntity.ArrayEntity<T>().apply(init)
    value = entity
